...
def test_output_shape(engine_cls):
    ...
    assert adv.shape == TEST_IMAGE.shape, \
        f"{engine_cls.__name__} output shape mismatch"
...
def test_output_dtype(engine_cls):
    ...
    assert adv.dtype == np.uint8, \
        f"{engine_cls.__name__} output dtype should be uint8"
...
@pytest.mark.parametrize("engine_cls", [
    FgsmEngine, PgdEngine, LsbEngine, DctEngine, GaussianEngine,
])
