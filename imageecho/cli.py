...
def print_report(report, elapsed: float):
    ...
    t.add_row(
        "Epsilon", f"{report.epsilon:.5f}  ({round(report.epsilon*255)}/255)")
...
def cmd_batch(args):
    ...
    engines_to_run = list(ENGINE_MAP.keys()) if args.engine == "all" else [
        args.engine]
        ...
        for img_path in images:
            ...
            for eng_name in engines_to_run:
                try:
                    ...
                    console.print(
                        f"[red]  {img_path.name} / {eng_name}: {e}[/red]")
                pbar.update(1)
...
Examples:
  ...
  imageecho batch ./images/ --engine all --epsilon 8 --output ./perturbed/
    ...
    p_run.add_argument(
        "--output", "-o", default=None, help="Output path (default: adv_<engine>.png)"
    )
    ...
    p_bench.add_argument(
        "--save-report", action="store_true", help="Save results to benchmark_report.md"
    )
    ...
    p_batch.add_argument(
        "--engine", "-e", default="fgsm", help="Engine name or 'all' (default: fgsm)"
    )
    p_batch.add_argument(
        "--epsilon",
        "-eps",
    ...
    p_batch.add_argument(
        "--output", "-o", default="perturbed", help="Output folder (default: perturbed)"
    )
