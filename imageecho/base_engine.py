...
class PerturbationReport:
    ...
    def __str__(self):
        ...
        return (f"[{self.engine_name}] eps={self.epsilon:.4f} "
...
class BaseEngine(ABC):
    ...
    def __init__(self, epsilon: float = 8/255, surrogate: Optional[SurrogateModel] = None):
    ...
    def apply(self, image: np.ndarray, target_class=None):
        ...
        return adv_image, PerturbationReport(
            engine_name=self.name, epsilon=self.epsilon,
            ssim=compute_ssim(image, adv_image, channel_axis=2, data_range=255),
            psnr=compute_psnr(image, adv_image, data_range=255),
            mean_delta=float(delta.mean()), max_delta=float(delta.max()),
            pixels_altered=int((delta.sum(axis=2) > 0).sum()),
            original_class=orig.class_id, original_confidence=orig.confidence,
            perturbed_class=adv_pred.class_id, perturbed_confidence=adv_pred.confidence,
