    ...
    def _perturb(
        ...
        for step in range(self.steps):
            ...
            if target_class is None:
                ...
                others = torch.cat(
                    [logits[0, :true_cls], logits[0, true_cls + 1:]])
                f_loss = torch.clamp(
                    true_logit - others.max() + self.kappa, min=0)
            else:
                # Targeted: pull toward target class
                tgt_logit = logits[0, target_class]
                others = torch.cat(
                    [logits[0, :target_class], logits[0, target_class + 1:]]
                )
                f_loss = torch.clamp(
                    others.max() - tgt_logit + self.kappa, min=0)
