...
class AutoPgdEngine(BaseEngine):
    ...
    def __init__(self, epsilon: float = 8/255, steps: int = 100,
                 n_restarts: int = 1, surrogate=None):
    ...
    def _run_once(self, x: torch.Tensor, target_class: Optional[int]):
        ...
        for i in range(self.steps):
            ...
            with torch.no_grad():
                ...
                cls    = torch.tensor(
                    [target_class if target_class is not None
            ...
            if plateau >= 10:
                ...
                plateau   = 0
                if i % 20 == 0:
                    print(f"  [AutoPGD] step {i}  step_size={step_size:.5f}  loss={loss:.4f}")
