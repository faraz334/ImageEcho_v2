...
class DeepFoolEngine(BaseEngine):
    ...
    def _perturb(
        ...
        print(
            f"  [DeepFool] original class: {orig_class}, "
            f"running up to {self.steps} steps..."
        )

        for step in range(self.steps):
            x_leaf = x_adv.clone().unsqueeze(0).requires_grad_(True)
            logits = self.surrogate.model(self.surrogate._normalize(x_leaf))
            ...
            top_k = torch.topk(
                logits.detach(), self.num_classes, dim=1).indices[0]
            ...
            x_adv = torch.clamp(
                x_adv, x.to(device) - self.epsilon, x.to(device) + self.epsilon
            )
